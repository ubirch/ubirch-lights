/*
 * File:    avrnacl_8bitc/crypto_sign/sc25519.c
...
/* All functions reduce completely */
...
static void barrett_reduction(unsigned char* r, unsigned char* a) 
{
  ...
  /* m has only 253 bits, so r fits into 32 bytes */
  bigint_sub(r, a, q1, 32);  
  
  /* Fully reduce, at most 2 subtractions are required */
  c = bigint_sub(q2, r, m, 32);
  bigint_cmov(r, q2, 1-c, 32);
  /* XXX: Can we prove that the second subtraction is not required? */
  c = bigint_sub(q2, r, m, 32);
  bigint_cmov(r, q2, 1-c, 32);
}
